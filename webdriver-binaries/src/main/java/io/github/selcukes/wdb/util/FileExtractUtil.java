    ...
    public static void extractFile(Path source, Path destination, DownloaderType compressedBinaryType) {
        ...
        try (var filesInDirectory = Files.list(destination)) {
            ...
            if (e instanceof NoSuchFileException) {
                throw new WebDriverBinaryException("The destination folder does not exist: " + extractedFile.toAbsolutePath(), e);
            } else {
                throw new WebDriverBinaryException("Error occurred while checking the destination folder: " + extractedFile.toAbsolutePath(), e);
            }
        }
